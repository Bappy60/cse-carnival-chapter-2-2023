...
function AllMembersData()
{
    ...
    useEffect(()=>{
        axios.get(`${baseURL}/superadmin/allmembers`).then(res=>{
            console.log(res);
            let rows=res.data;
    ...
    {
        value:"hours",label:"Hours Sponsored"
    },
...
return(
  ...
  {enntries.map(e=>{
    return (<MenuItem key={e}value={e.value}>{e.label}</MenuItem>)
     ...
     {data.length>0&&  <BasicTable head={[{value:"Full Name",label:"name"},{value:"Email Address",label:"email"},{value:"Hours Used",label:"hours"},{value:"Hours Sponsored",label:"hours"},{value:"Onboarding Date",label:"onboardDate"},{value:"Company Name",label:"companyName"},]} rows={data} type={"member"}/>}
     </Box>
    </div>
)
}

export default AllMembersData;