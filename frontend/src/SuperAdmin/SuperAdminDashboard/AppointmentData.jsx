...
function AppointmentData()
{
    ...
    useEffect(()=>{
        axios.get(`${baseURL}/superadmin/getevents`).then(res=>{
            console.log(res);
            let rows=res.data;
...
return(
  ...
  {enntries.map(e=>{
    return (<MenuItem key={e}value={e.value}>{e.label}</MenuItem>)
     ...
     {data.length>0&&  <BasicTable head={[{value:"Id",label:"_id"},{value:"Member Id",label:"id"},{value:"Care Provider Id",label:"careProviderId"},{value:"Date & Time",label:"start_time"},{value:"Created On",label:"created"}]} rows={data} type={"appointment"}/>}
     </Box>
    </div>
)
}
