...
export default function ShowAllDetails({ event }) {
  ...
  const submit = () => {
    axios
      .put(`${baseURL}/careprovider/updatescarenotes`, {
        id: event._id,
        careNotes,
      })
      .then((res) => {
  ...
  useEffect(() => {
    axios
      .get(`${baseURL}/members/profile?id=${event.id}`)
      .then((result) => {
      ...
      {Object.keys(profile).length > 0 && (
        <Box>
          <Stack direction="row" sx={{ gap: "20px" }}>
            <img
              src={
                !profile.images ? carep : `${baseURL}/images/${profile.images}`
              }
            ...
            <Box sx={{ textAlign: "start" }}>
              ...
              <p style={{ margin: "0px", padding: "0px" }}>
                {age(new Date(profile.dob))} years old
              </p>
            </Box>
          </Stack>
          ...
          <textarea
            rows={10}
            placeholder="Add Notes to myself"
            value={careNotes}
            onChange={(e) => setcareNotes(e.currentTarget.value)}
            style={{
              width: "90%",
