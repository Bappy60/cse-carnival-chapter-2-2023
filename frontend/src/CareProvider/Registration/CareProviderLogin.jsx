...
function CareProviderLogin() {
  ...
  const handlePasswordChange = (e) => {
    ...
    if (email.length > 0) setError(false);
    else setError(true);
  ...
  const submit = (e) => {
    e.preventDefault();
    axios
      .post(`${baseURL}/careprovider/login`, {
        email,
        password,
      })
      .then((result) => {
  ...
  return (
    <div style={{ width: "100%", paddingTop: "10rem" }}>
      ...
      <form style={{ width: "25%", margin: "0px auto" }}>
        ...
        {dberror && (
          <p style={{ color: "red" }}>
            No such email and password combintation found
          </p>
        )}
        <button
          onClick={error ? "" : submit}
          style={{
            backgroundColor: error ? "#D9D9D9" : "#62C227",
            color: "white",
            padding: "15px 55px",
            marginTop: "30px",
          }}
