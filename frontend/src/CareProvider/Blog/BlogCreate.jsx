...
export default function CreateBlog() {
  ...
  const submit = (event) => {
    ...
    if (data.title && data.desc) {
      ...
      axios
        .post(`${baseURL}/blog/createblog`, dat)
        .then(
          (res) => {
            console.log(res);
            setSuccess(true);
  ...
  return (
    <Box>
      ...
      >
        <h3>
          Write an article so that the patients can learn things about various
          things of Pyscology
        </h3>
        {rows.map((row, index) => {
          return (
        ...
        <label
          htmlFor="name"
          style={{ textAlign: "start", display: "block", marginBottom: "5px" }}
        >
          Choose which type of the <span style={{ color: "red" }}>*</span>
        </label>
        ...
        {file === "" && (
          <img src={psy} style={{ width: "500px", height: "500px" }} />
        )}
        {file !== "" && (
          <div>
        ...
        {error && (
          <p style={{ color: "red" }}>All fields has to be submitted</p>
        )}
        <button
          onClick={submit}
          style={{
