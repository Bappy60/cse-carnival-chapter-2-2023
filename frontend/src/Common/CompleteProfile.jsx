...
function CompleteProfile({ type, nav }) {
  ...
  const rows = [
    ...
    { value: "Years of Experience", label: "experience" },
    { value: "Practice License Number", label: "license" },
    { value: "Scheduling Link", label: "scheduleLink" },
  ];
  useEffect(() => {
    axios
  ...
  const submit = (e) => {
    ...
    axios
      .put(`${baseURL}/careprovider/editprofile`, dat)
      .then(
        (res) => {
          console.log(res);
          setSuccess(true);
                ...
                {rows.map((row, index) => {
                  return (
                    <div key={row}>
                      ...
                      <input
                        type={row.label === "experience" ? "number" : "text"}
                        value={profile[row.label]}
                        min={0}
  ...
  { Object.values(languages()).map((l) => <MenuItem key={l} value={l.name}>{l.nativeName}</MenuItem>)}
                ...
                <textarea
                  value={profile.bio}
                  onChange={(e) =>
                    setProfile({ ...profile, bio: e.currentTarget.value })
                  }
                  rows={10}
                ...
                {expError && (
                  <p style={{ color: "red" }}>
                    Experience cannot be less than 0
                  </p>
                )}
                <div
                  style={{
